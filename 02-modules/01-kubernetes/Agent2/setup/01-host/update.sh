...
AGENT2_PUBLIC_IP=$(curl -s ifconfig.me)
AGENT2_PRIVATE_IP=$(hostname -I | cut -f 1 -d " ")
AGENT2_DOMAIN_NAME="agent2.domainname.com"
AGENT2_ZEROTIER_IP=$(echo $(hostname -I) | cut -d " " -f 2)

MATCHES_IN_HOSTS="$(grep -n $AGENT2_PUBLIC_IP /etc/hosts)"
hostnamectl set-hostname $AGENT2_DOMAIN_NAME
