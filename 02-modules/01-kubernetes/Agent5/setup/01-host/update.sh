...
AGENT5_PUBLIC_IP=$(curl -s ifconfig.me)
AGENT5_PRIVATE_IP=$(hostname -I | cut -f 1 -d " ")
AGENT5_DOMAIN_NAME="agent5.domainname.com"
AGENT5_ZEROTIER_IP=$(echo $(hostname -I) | cut -d " " -f 3)

MATCHES_IN_HOSTS="$(grep -n $AGENT5_PUBLIC_IP /etc/hosts)"
hostnamectl set-hostname $AGENT5_DOMAIN_NAME
