...
AGENT4_PUBLIC_IP=$(curl -s ifconfig.me)
AGENT4_PRIVATE_IP=$(hostname -I | cut -f 1 -d " ")
AGENT4_DOMAIN_NAME="agent4.domainname.com"
AGENT4_ZEROTIER_IP=$(echo $(hostname -I) | cut -d " " -f 3)

MATCHES_IN_HOSTS="$(grep -n $AGENT4_PUBLIC_IP /etc/hosts)"
hostnamectl set-hostname $AGENT4_DOMAIN_NAME
